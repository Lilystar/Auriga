...
#	2006.09.07 インジェクション攻撃を受けるセキュリティホールを修正 byよらん
# 
#
# ** 設定方法 **
#
...
if($ENV{'HTTP_ACCEPT_LANGUAGE'}=~/^ja/){
	my(%tmp)=(
		...
		'Account \[(.+)\] creation failed\. same account exists.*'
			=> 'アカウント "$1" は既に存在します.',
		'Illegal charactor found in UserID.*'
			=> 'IDの中に不正な文字があります.',
		'Illegal charactor found in Password.*'
...
if( $cgi->param("addaccount") ){
	...
	#入力項目チェック
	if($cgi->param("userid")=~/[^0-9A-Za-z\@\_\-\']/){
		HttpError("Illegal charactor found in UserID.");
	}
	if($cgi->param("passwd")=~/[^0-9A-Za-z\@\_\-\']/){
		HttpError("Illegal charactor found in Password.");
	}
	if(lc(substr($cgi->param("gender"),0,1))=~/[^0-9A-Za-z\@\_\-\']/){
...
#	}
	...
	my($command) = "$perl $ladmin --add $userid $gender $passwd";
	my(@msg) = `$command`;
