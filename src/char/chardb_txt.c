...
static int mmo_char_tostr(char *str,struct mmo_chardata *p)
{
	...
	str_p += sprintf(str_p,"%d\t%d,%d\t%s\t%d,%d,%d\t%d,%d,%d\t%d,%d,%d,%d\t%d,%d,%d,%d,%d,%d\t%d,%d"
		...
		p->st.costume_head_top,p->st.costume_head_mid,p->st.costume_head_bottom,
...
static int mmo_char_fromstr(char *str,struct mmo_chardata *p)
{
	...
	p->st.costume_head_top    = tmp_int[38];
	p->st.costume_head_mid    = tmp_int[39];
	p->st.costume_head_bottom = tmp_int[40];
	p->st.last_point.x        = tmp_int[41];
	p->st.last_point.y        = tmp_int[42];
	p->st.save_point.x        = tmp_int[43];
	p->st.save_point.y        = tmp_int[44];
	p->st.partner_id          = tmp_int[45];
	p->st.parent_id[0]        = tmp_int[46];
	p->st.parent_id[1]        = tmp_int[47];
	p->st.baby_id             = tmp_int[48];
	p->st.delete_date         = (unsigned int)tmp_int[49];
