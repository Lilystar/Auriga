...
const struct mmo_chardata* chardb_sql_load(int char_id)
{
	...
	if (sql_res) {
		...
		p->st.robe                = atoi(sql_row[39]);
		p->st.costume_head_top    = atoi(sql_row[40]);
		p->st.costume_head_mid    = atoi(sql_row[41]);
		p->st.costume_head_bottom = atoi(sql_row[42]);
		strncpy(p->st.last_point.map,sql_row[43],24);
		p->st.last_point.x        = atoi(sql_row[44]);
		p->st.last_point.y        = atoi(sql_row[45]);
		strncpy(p->st.save_point.map,sql_row[46],2);
		p->st.save_point.x        = atoi(sql_row[47]);
		p->st.save_point.y        = atoi(sql_row[48]);
		p->st.partner_id          = atoi(sql_row[49]);
		p->st.parent_id[0]        = atoi(sql_row[50]);
		p->st.parent_id[1]        = atoi(sql_row[51]);
		p->st.baby_id             = atoi(sql_row[52]);
		p->st.delete_date         = atoi(sql_row[53]);
		p->st.refuse_partyinvite  = atoi(sql_row[54]);
		p->st.show_equip          = atoi(sql_row[55]);
...
const struct mmo_chardata* chardb_sql_make(int account_id,unsigned char *dat,int *flag)
{
	...
	do
	{
		...
		sqldbs_stmt_bind_param(&bind[4],SQL_DATA_TYPE_INT,INT2PTR(&str),0,0,0);
		sqldbs_stmt_bind_param(&bind[5],SQL_DATA_TYPE_INT,INT2PTR(&agi),0,0,0);
		sqldbs_stmt_bind_param(&bind[6],SQL_DATA_TYPE_INT,INT2PTR(&vit),0,0,0);
		sqldbs_stmt_bind_param(&bind[7],SQL_DATA_TYPE_INT,INT2PTR(&int_),0,0,0);
		sqldbs_stmt_bind_param(&bind[8],SQL_DATA_TYPE_INT,INT2PTR(&dex),0,0,0);
		sqldbs_stmt_bind_param(&bind[9],SQL_DATA_TYPE_INT,INT2PTR(&luk),0,0,0);
		...
		sqldbs_stmt_bind_param(&bind[14],SQL_DATA_TYPE_INT,INT2PTR(&hair),0,0,0);
		sqldbs_stmt_bind_param(&bind[15],SQL_DATA_TYPE_INT,INT2PTR(&hair_color),0,0,0);
