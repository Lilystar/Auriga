...
int guild_readdb(void)
{
	...
	i=0;
	while(fgets(line,256,fp) && i<MAX_GUILDLEVEL){
		if(line[0]=='/' && line[1]=='/')
			continue;
...
// ギルドデータの文字列への変換
int guild_tostr(char *str,struct guild *g)
{
	int i,c,len;
	// 基本データ
...
// ギルドデータのセーブ用
int guild_txt_sync_sub(void *key,void *data,va_list ap)
{
	char line[16384];
	FILE *fp;
...
void guild_txt_config_read_sub(const char* w1,const char *w2) {
	if(strcmpi(w1,"guild_txt")==0){
		...
		strncpy( guild_journal_file, w2, sizeof(guild_journal_file) );
	}
		...
		strncpy( guildcastle_journal_file, w2, sizeof(guildcastle_journal_file) );
	}
...
const struct guild *guild_sql_load_num(int guild_id) {
	...
	if (sql_res!=NULL && mysql_num_rows(sql_res)>0) {
		...
		for(i=0; (sql_row = mysql_fetch_row(sql_res)) && i<MAX_GUILD ;i++){
			...
			m->online=(unsigned char)atoi(sql_row[10]);
...
const struct guild* guild_sql_load_str(char *str) {
	...
	sprintf(
		tmp_sql,"SELECT `guild_id` FROM `%s` WHERE `name` = '%s'",
...
int  guild_sql_save(struct guild* g2) {
	...
	// printf("]\n");
	{
		struct guild* g3 = numdb_search(guild_db,g2->guild_id);
		if(g3)
...
int update_castle(int guild_id,int index)
{
	struct guild_castle *gc=numdb_search(castle_db,index);
	if(gc==NULL){
		return 0;
	}
...
int mapif_parse_GuildSkillUp(int fd,int guild_id,int skill_num,int account_id,int flag)
{
	...
	if (g1 == NULL || idx < 0 || idx >= MAX_GUILDSKILL)
		return 0;
	memcpy(&g2,g1,sizeof(struct guild));
	if( (g2.skill_point>0 || flag&1) && g2.skill[idx].id>0 && g2.skill[idx].lv<10) {
		g2.skill[idx].lv++;
